...
\emph{Uniform deviation.} Let
$Z=\sup_{\|w\|_2\le W}|\widehat R_j(w)-R_j(w)|$ over the validation sample of
size $m$. Changing one sample moves $Z$ by at most $L_\infty/m$, so
McDiarmid's inequality gives
$Z\le\mathbb E Z+L_\infty\sqrt{\log(2q/\delta)/(2m)}$ with probability at
least $1-\delta/q$ (we use the two-sided form for a margin). By
symmetrization, $\mathbb E Z\le 2\,\mathfrak R_m(\mathcal L)$, the Rademacher
complexity of the loss class. The loss is the composition of $t\mapsto t^2$,
...
\[
Z\;\le\;\frac{4Wb^2D(1+WD)}{\sqrt m}
+L_\infty\sqrt{\frac{\log(2q/\delta)}{2m}}
\qquad\text{with probability}\ge 1-\delta/q .
\]
...
\le\widehat R_j(w^\ast)+Z\le R_j(w^\ast)+2Z$, and $2Z$ is bounded by the
displayed excess. A union bound over the $q$ coordinates makes all events
...
symmetrization doubles it, and McDiarmid at level $\delta/(q|\Gamma|)$ adds
$\tilde c^{\,2}\sqrt{\log(2q|\Gamma|/\delta)/(2m)}$. A union bound over the
$q|\Gamma|$ pairs makes, with probability at least $1-\delta$, every
deviation
$Z_{j,\gamma}=\sup_{\|w\|\le W}|\widehat R_{j,\gamma}(w)-R_{j,\gamma}(w)|$
at most
\[
\frac{4W b^2D(1+WD)(1+\kappa)^2}{\sqrt m}
+b^2(1+WD)^2(1+\kappa)^2\sqrt{\frac{\log(2q|\Gamma|/\delta)}{2m}} .
\]
...
The argument has five steps: a pointwise error bound whose design dependence
