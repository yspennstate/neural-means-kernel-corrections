...
(Lemma~\ref{lem:kappa}, $\kappa_{\mathrm{an}}=10^{3}$). All are
capacity bounds, vacuous at the shipped constants and reported for
their scaling; the one statement whose evaluated value is the
quantity reported is Theorem~\ref{thm:or} below.
...
On the structural-mechanics benchmark the measured spreads put that window
at roughly a fifth of the squared floor, and the best convex mixture of
the measured $S$ sits at the bottom of it: $\predRMS\pm\predRMSSd\%$
against a floor of $\ensFloor\pm\ensFloorSd\%$, both in the
root-mean-square metric $\mathcal E_2$ the corollary is stated in.
Supplement~\ref{supp:experiments} evaluates the bracket at every seed.
