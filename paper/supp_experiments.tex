...
$\saKfMlpOne\%$ at weight $1.0$ in the smaller campaign.
...
regularization preferences of the fitted corrections.
...
\label{sec:cost}
The pipeline was developed on a single workstation GPU and replicated on
four cloud CPU instances. The kernel stages use dense, exact linear solves. At the campaign's
...
The fitting stages use validation-based choices. The release contains splits, code and per-seed
...
These associations describe the measured data. Ensemble disagreement has correlation
...
Within the two campaigns, per-case paired differences isolate changes
...
holding at all ten. These describe repeatability across seeds and paired test-block effects.
...
predictor, so an RMS lower bound does not transfer to mean error. Its per-pixel affine weights and kernel correction also place it
...
heuristic equicorrelation references. The dispersion factors
...
the weights. They are same-sample metric diagnostics. For six members the ratio of test mean error to validation RMS
...
These measurements describe the variability of the fitted stages.
