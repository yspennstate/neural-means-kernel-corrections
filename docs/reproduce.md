...
theoretical results (exact-norm residual for the conditional power-function
bound, the leave-half-out identity, the effective-dimension and
Marchenko-Pastur formulas, the anisotropy rate mechanism);
`campaign/eval_kappa.py` measures the correction-weight norm entering the
capacity bound of the grid-tuned correction. Conformal calibration everywhere
uses a random subset of the test block that no selection step touched,
evaluated on the disjoint remainder.

## Environment and data checksums

The complete-schedule campaign and every after-campaign check ran in one
Python 3.10.12 virtual environment on a 40-core Xeon E5-2698 v4 host (NumPy
2.2.6, SciPy 1.15.3, PyTorch 2.13.0 CPU); `pip freeze` of that environment is
`campaign/collected/dgx/requirements_lock_nmkc_venv.txt`. SHA-256 prefixes of
the data files as used:

| file | sha256 (first 16 hex) |
|---|---|
| structural mechanics `loads.npy` | 8baf46f02fcbab5e |
| structural mechanics `stress.npy` | a3c060c4f58aaefa |
| structural mechanics `idx_train.npy` / `idx_test.npy` | 09de6da025d690dd / b5181aaef47f8ec6 |
| OCO-2 `dimred_data_4_mono.jld` | 01a1e8fe3a60e8d4 |
| OCO-2 `dimred_variables_4_mono.jld` | d979ef32270ac4d7 |
| OCO-2 `kf_results_o2_4_mono.jld` / `wco2` / `sco2` | ef1bdf80a02e1397 / a806aaf76782da2b / 3eea6a5ccfdad37e |
| Helmholtz inputs / outputs | 56c7b8d80487df70 / 739dd9c13761596e |
| Navier-Stokes inputs / outputs | 1e17cb82eeb5ace0 / a40b5fdeccbe19b8 |

The seeded scripts read `NMKC_SPLIT_SEED` for every split, so a table row is
regenerated by the command listed above it with the seed of the row; the
per-seed run records under `campaign/collected/` carry the selected
hyperparameters, checkpoint epochs and stage decisions the text reports.

## Second review (3 September 2026): what changed and where

A second referee panel reviewed commit fb3a197. The revision made for it,
and the record behind each change:

- Every quantity of Proposition 6.1 on OCO-2 is now computed in the
  proposition's own terms: root-mean-square relative errors and the
  uncentered alignment S_12/(e_1 e_2), from the rerun's member prediction
  arrays. `campaign/dgx_checks/oco_ensemble_recheck.py` writes
  `campaign/collected/dgx/oco_ensemble_recheck.json` (per-seed admission
  example, all 840 scoreboard rows with both the existence and the shipped
  readings, and the E1/E2 errors of every member). The scoreboard's test-side
  outcome is scored against the better member on test.
- The structural-mechanics ensemble alignments are the uncentered ones;
  `audit_reported_macros.py` recomputes them from the stored second-moment
  matrices (`unc()`), which moved two macros by at most 0.002.
- Figure S6.4 is regenerated from the seeded test-block calibration path
  (`uq_fig_dump.py` on the host, `fig_uq_seeded.py` here) and its calibration
  panel is against absolute error; the coverage bands in the text are the
