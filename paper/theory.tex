...
bound, stated and proved in the online supplement
(Section~S1): a symmetrization inequality behind the reflection
...
validation scores $\|e(\tilde u_i)\|_2/P_\lambda(\tilde u_i)$. Its coverage
needs neither the bound to be tight nor $P_\lambda$ to rank the error: it
needs exchangeability alone, which the fixed splits provide, and it then
holds in finite samples, $1-\alpha\le\mathbb P(G(u^\star)\in C(u^\star))\le
1-\alpha+1/(m+1)$ (Proposition~\ref{prop:conf}). The realized coverage has
...
the reason this is the one uncertainty signal on this benchmark that
survives testing, are in Supplement~\ref{supp:conf}.
