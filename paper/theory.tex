...
($\varrho\in[0.86,0.96]$ across member pairs at member errors within a few
percent of one another) put the window at roughly a tenth of the squared
floor, which is precisely the margin by which the fitted stack sits above
$\bar e\sqrt{\bar\varrho}$ in Section~\ref{sec:experiments}; the seed
campaign reports the bracket alongside the realized ensemble risk for every
seed and both problems.
...
The seed campaign tests the rate half of this prediction directly: both
kernel rows are refit at nested training sizes over a sixteenfold range,
where the proposition requires the two empirical exponents to coincide on
this near-full-rank input with a roughly constant gap, in contrast to the
controlled rank-gap target of the verification suite, where the adapted
metric must improve the exponent itself.
...
$m=1000$ and $\alpha=0.1$. Any gap to nominal is not the
...
standard deviation $\sqrt{\alpha(1-\alpha)/m}$ is about a point at these
sizes; Remark~\ref{rem:covlaw} gives the exact law of that fluctuation, and
the seed campaign places every seed's observed coverage against it. The hypotheses
