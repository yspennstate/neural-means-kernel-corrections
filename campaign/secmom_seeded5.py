...
OUT_JSON = os.environ.get("NMKC_SECMOM_OUT", "")   # set to write the record
RECORDS = []
...
for sd in sorted(glob.glob(os.path.join(ROOT, "seeds", "sm_s*"))):
    ...
    # Solve the simplex QP properly. ensemble_theory.py used projected gradient with
    # clip-and-renormalise, which is not a projection onto the simplex: it converges to
    # near-vertex points and sat 0.8-3.6% above the true minimum on this five-member S,
    # reporting "all weight on one member" where the optimum spreads across all five.
    # Every weight vector and predicted risk taken from that routine is wrong.
    ...
    # Keep/drop, by name rather than by position. The earlier four-member version
    # hardcoded index 3 as "the kernel"; with the FNO inserted before it, index 3 is the
    # FNO and every line so labelled was reporting the wrong member. Resolve the index
    # from NAMES and print what was actually compared.
    net_idx = [NAMES.index(n) for n in ("mlp", "mlpMSE", "mlpR") if n in NAMES]
    bi = min(net_idx, key=lambda i: e[i])          # most accurate residual-MLP member
    parts = []
    for cand in ("fno", "krr"):
        ...
        rho_c = C[bi, ci]
        thresh = e[bi] / e[ci]
        parts.append("%s vs %s: rho %.3f vs e1/e2 %.3f -> %s"
                     % (cand, NAMES[bi], rho_c, thresh,
                        "KEEP" if rho_c < thresh else "DROP"))
    print("s%-2s w=%s  pred %.4f  real val %.4f test %.4f  disp %.3f | "
          "rho[min %.3f mean %.3f max %.3f] ebar %.4f floor %.4f | %s"
          % (seed, dict(zip(NAMES, np.round(w, 3))), pred, real_va, real_te,
             real_va / pred, min(off), rbar, max(off), ebar, floor, " | ".join(parts)))

    # The paper quotes predRMS, dispFac, corrBar and ensFloor from these
    # numbers, so they are recorded rather than only printed.
    RECORDS.append(dict(
        seed=int(seed), weights=dict(zip(NAMES, [float(x) for x in w])),
        pred_rms=float(pred), real_val=float(real_va), real_test=float(real_te),
        disp_factor=float(real_va / pred), rho_min=float(min(off)),
        rho_mean=float(rbar), rho_max=float(max(off)),
        ebar=float(ebar), floor=float(floor),
        member_err={n: float(v) for n, v in zip(NAMES, e)}))
...
if OUT_JSON and RECORDS:
    ...
    print("set NMKC_SECMOM_OUT to write the per-seed record to disk")
