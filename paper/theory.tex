...
(Appendix~\ref{app:verification}). The convexity argument behind the
...
\frac{P_\lambda(u)}{\sqrt\lambda}\;\le\;\sqrt{\frac{k_{\max}}{\lambda}} ,
\]
where $P_\lambda(u)^2=k(u,u)-k(u,X)(K+n\lambda I)^{-1}k(X,u)$ is the
posterior variance of Section~\ref{sec:theory-or}, equation~\eqref{eq:power}.
In particular, for the residual correction of Section~\ref{sec:method-kernel}
--- a Mat\'ern kernel with $k(u,u)=1$ and the nugget grid
$\lambda\in\{10^{-6},10^{-5},10^{-3}\}$ --- the hypothesis of
Corollary~\ref{cor:certificate} holds, whatever the length scale, with
\[
\kappa_{\mathrm{an}}\;:=\;\max_{\gamma\in\Gamma}\lambda_\gamma^{-1/2}\;=\;10^{3}.
\]
\end{lemma}

The proof is two lines in Appendix~\ref{app:proofs}, and the bound is the
right kind of constant for the corollary: it depends on nothing that is
estimated. It is also why the corollary is a capacity bound and not a
certificate. The excess term carries $(1+\kappa)^2$, so at
$\kappa_{\mathrm{an}}=10^3$ the corollary's right-hand side is, like that of
...
grid averages $31.2$ across three seeds (Appendix~\ref{app:verification}),
about a thirtieth of $\kappa_{\mathrm{an}}$, so that evaluating the display
at the probe value would shrink it by a factor of close to a thousand. But a
supremum over a finite probe is a lower bound on the constant the corollary
needs, not the constant, and we do not use it as one. What the corollary
...
closed form. The certificate of this paper, in the sense of a bound whose
evaluated value is the quantity reported, is Theorem~\ref{thm:or}.
...
Appendix~\ref{app:proofs} records the argument at that level, as a sketch
...
The calculation (Appendix~\ref{app:proofs}) is the scattered-data estimate
...
quantity we compute: the reported coverage of $\uqCover\pm\uqCoverSd\%$ at the
nominal $1-\alpha=\uqCoverNom\%$ in Section~\ref{sec:uq} realizes this
proposition with $m=1000$ and $\alpha=0.1$. Any gap to nominal is not the
...
is $\uqCoverSd$ points against the $0.9$ points the law predicts, and every
...
coverage is evaluated on the disjoint remainder (Appendix~\ref{app:impl}), so
