On the structural-mechanics benchmark the neural mean and the kernel
contribute comparably to the final accuracy. The radiative-transfer emulation
problem of \citet{susiluoto2025radiative} behaves differently, and we apply
the same components to it: per
...
reports that the two sets share no point. The runs split the pool
...
the coordinate winners below. The public test states are used for the reported
...
test block on O2, so every selection made on the carve is made under a shift between the two.
The comparison is therefore computed on identical test points against the published emulator's own predictions.
...
subsample, and the winner refit on all $18000$ training points. This matches the kernel stage; feature heads additionally require neural-network training. The
...
the penalty chosen on validation, and by the exact Mat\'ern head. Entries are
computed by \texttt{campaign/jpl\_seeded.py}; the network and head rows
reproduce Table~\ref{tab:oco2} within the seed spread.}
\label{tab:oco2ridge}
...
weighted losses buy the leading coefficient at the price of the tail, and by almost the same amount ($0.0012$ and $0.0016$); the per-coordinate combination takes
...
error on that coordinate; we also ran a selector
...
$2000$-state validation carve. Instrument response, Jacobians and the
retrieval itself are outside its scope. Learning the metric instead of setting it by hand does not change this picture. A per-dimension metric fit by
...
checks the algebraic identity against stored predictions.
...
$100\%$. That scores the existence of a helpful mix. Scoring instead the
...
the rule's yes-or-no prediction about the transferred mixture is right; in
...
(\texttt{oco\_ensemble\_recheck.json} in the release). The $28$ pairs of a band-seed share members and the ten seeds of a band
share one test block, so the bins describe a trend across margin rather
than binomial samples. In the small-margin pairs it is the transfer of the two estimates from one block to another that degrades.
