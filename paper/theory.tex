...
one ($0.032$ and $0.035$ points in the two campaigns) is the variation of the
weights over the grid, not their sign.
...
finite-design diagnostics of the interpolating regime: the fitted norm is a
lower bound on the unknown target norm, the ratio grows with the kernel
...
One caveat precedes the comments. In the structural pipeline the
correction is trained on residuals of the cross-fitted mean,
$G(X)-m_{\mathrm{cf}}(X)$, in which each training row's members are
out-of-fold, and is deployed on top of the full-data mean, as
$m_{\mathrm{full}}(u)+\widehat r_\lambda(u)$. The theorem, stated for one
fixed $m$, therefore covers the pair $(m_{\mathrm{cf}},\widehat r_\lambda)$
on the training design; for the shipped estimator the bound acquires a term
in $m_{\mathrm{cf}}(X)-m_{\mathrm{full}}(X)$, the two means' disagreement on
the training rows, which this version does not measure. Three comments.
First, the inequality is algebraic: it holds pathwise for
...
(Proposition~\ref{prop:conf}). The coverage conditional on the
calibration sample has an exact law as well --- $\mathrm{Beta}(k,m+1-k)$ at
$k=\lceil(1-\alpha)(m+1)\rceil$ --- and the fraction observed on $N$
evaluation cases is Beta--binomial, whence the $[88.0\%,91.8\%]$ band at
$m=1000$, $N=19000$ against which Section~\ref{sec:uq} places the ten seeds
(Remark~\ref{rem:covlaw}). Both statements, with their proofs and
