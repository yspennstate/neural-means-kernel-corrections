...
% OCO-2 alignment diagnostic recomputed after review (campaign/dgx_checks/jpl_alignment_check.py, O2, seed 0,
...
\newcommand{\specCompsHi}{57}           % most components above the edge (the pruned MSE network)
...
\newcommand{\normRatio}{4900}           % raw over residual, fitted squared RKHS norms (was 271 at the original single-seed nugget)
...
\newcommand{\costGramS}{26}             % block-filled Gram build, seconds
\newcommand{\costCholS}{26}             % in-place Cholesky, seconds
\newcommand{\costSolveS}{12}            % triangular solves with 1681 right-hand sides, seconds
\newcommand{\costPeakBlockGb}{6.0}      % peak resident memory of the block-filled path, GB
\newcommand{\costPeakNaiveGb}{19.2}     % peak resident memory after the naive full-matrix build, GB
\newcommand{\costQueryOneMs}{18}        % kernel-stage latency, single query, milliseconds
\newcommand{\costQueryBatchMs}{1.9}     % kernel-stage latency per query at a batch of 1000, milliseconds
