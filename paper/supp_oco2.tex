...
participation ratios are $11.99$, $13.63$ and $13.33$. These are descriptive summaries of an estimated linear sensitivity map.
...
hyperparameters. This is a consistency check on the two code paths: both call the same tuning routine on the same standardized inputs. One
...
$3\times10^{-17}$ and $1\times10^{-17}$ on the three bands --- the proposition's content, checked against real predictions.
...
improves from $0.07927\%$ to $0.02940\%$. The result is therefore a trade-off. WCO2 has a weaker radiance
...
training slice at every size, so the budget was fixed while the data grew. What the
