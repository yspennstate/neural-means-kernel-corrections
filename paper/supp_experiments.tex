...
normalized-MSE network ($0.02$ points). In the ten-seed refiner records,
...
training does not make the averaging effect vanish. The additional
equivariance condition needed to apply Proposition~\ref{prop:sym} to this
refiner is not established; these changes are empirical measurements.
...
\label{sec:spectra}
Figure~\ref{fig:spectra} reports a separate diagnostic on $6000$ rows sampled
...
and the marked nugget is $10^{-5}$. These match the parameters recorded for
the earlier hybrid diagnostic; they are not a new selection or the parameters
of every deployed correction. The split and sampling generators both use seed~0.

The leading eight eigenvalues carry $99\%$ of the trace, while
$d_{\mathrm{eff}}(10^{-5})=103.11885$. Spectral mass and effective dimension
summarize different aspects of the matrix: the latter counts each mode with
weight $\mu_i/(\mu_i+6000\lambda)$. An independent Cholesky calculation of
$6000-6000\lambda\operatorname{tr}\big((K_S+6000\lambda I)^{-1}\big)$ agrees with the
eigenvalue sum to $1.7\times10^{-11}$. The directory
\mbox{\path{campaign/collected/spectrum_20260906}} retains the selected rows,
normalization, eigenvalues and generation record. This regenerated diagnostic
does not measure the full deployed smoother. Its spectral concentration also
does not reduce the cubic work of the dense factorization used here.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figs/spectra.pdf}
\caption{A regenerated $6000$-row training-subsample diagnostic with the fixed
kernel parameters specified in the text. Left: eigenvalues of $K_S/6000$
on a log scale. Right: the effective dimension from Lemma~\ref{lem:deff};
the marker at the fixed nugget $10^{-5}$ gives $103.12$. Both panels describe
this subsample matrix. The eigenvalues and an independent direct trace check
are included in the numerical record.}
\label{fig:spectra}
\end{figure}
...
instances. The kernel stages use dense, exact linear solves. At the campaign's
shapes, an eight-thread timing diagnostic measured $\costGramS$ seconds to
...
synthetic; the dimensions and operations match the campaign. The factorization
performs $n^3/3\approx2.3\times10^{12}$ floating-point operations. Low effective
dimension describes the regularized spectrum but does not reduce this dense
factorization's arithmetic cost. No GPU, inducing points, preconditioner or
...
exhausted a $31$\,GB instance during the campaign. The block-filled timing
...
full-matrix construction. This is a demonstrated implementation option:
the main correction driver still forms full-size temporaries and needs
the corresponding memory. The neural means have a few million parameters and include both metric-loss
...
\texttt{campaign/conformal\_seeded.py}. Here the constant-radius band is
...
the correction selected, and most of the raw ratio is amplitude: per unit energy the
...
\texttt{collected/dgx/sm\_norm\_check\_hpix\_s}\{\texttt{0,1,2}\}\texttt{.json}.}
\label{tab:norms}
\begin{adjustbox}{max width=\linewidth}
\begin{tabular}{lccc}
\toprule
...
and convex optima differ by at most $0.00052$ percentage points in the stored
