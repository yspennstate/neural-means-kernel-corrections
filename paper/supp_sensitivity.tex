...
original campaigns: target centering in out-of-fold kernel fits,
...
pooled predictions reproduce the original out-of-fold field before
...
predictor are held fixed. Thus the contrast includes propagation through the
...
block, ten paired seeds. The original pipeline is shown as a reference;
...
For each original six-member pipeline, we reconstruct the training-row
...
fixed. We then solve for two sets of coefficients: one from the original
...
\begin{table}[t]
\centering
\caption{Correction-label sensitivity, ten original mechanics pipelines.
No neural training or stack-weight fitting is repeated. The kernel is the
one selected in the original run.}
...
The original grid has scale multipliers $\{0.5,1,2,4\}$ and nuggets
...
per seed), $\sensOtwoOldBoundary$ meet a boundary of the original grid and
...
For the ten original corrected pipelines, the plain-grid error is
