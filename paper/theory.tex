...
Every fitted stage of the pipeline carries an elementary finite-sample
bound, stated and proved in the online supplement (Section~S1). The
bounds are stated for the pipeline as deployed: every selection --- the
member set, the stack weights, the kernel scale and nugget, and the choice
between the stacked and the corrected surrogate --- is made on the
validation split, the test block is read once, and the conformal
calibration split is drawn from the test block after all selections are
fixed, so the independence that the coverage statement needs holds by
construction rather than by assumption. The bounds are: a symmetrization inequality behind the reflection
...
licenses the per-coordinate combination of Section~\ref{sec:oco2}, and it
is why that combination can improve on every single model on both metrics
at once when different models win different coordinates.
...
in the worst case over the ball, and the price of a positive nugget in the
certificate is explicit.

\begin{theorem}[optimality of the certificate]\label{thm:minimax}
...
A_\lambda^{-2}K^{-1}k_u ,
\]
so the nugget's cost in the certificate is a computable quantity that
...
the certificate $\|G-m\|_K\,P_0(u)$ is not merely valid but the smallest
constant any method could certify from these data, and at the validated
