...
Two departures from campaign/correct_stack.py, both deliberate:
  ...
  * FIVE MEMBERS. The FNO is included here, finalised from its campaign checkpoint by
    finalize_fno.py; the UNet is not, because the wall clock cut the campaign inside the
    FNO stage and the UNet never started, so no checkpoint of it exists at any seed.
    Whether this five-member stack is preferred to the four-member one is decided on
    VALIDATION, per seed, exactly as every other stage choice in this pipeline.
