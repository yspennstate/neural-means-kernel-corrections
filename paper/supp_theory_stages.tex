...
symmetry of the distributed solver output. In the campaign, the recorded
averages improve twenty-nine of thirty members across the two MLP variants
and the refiner, and worsen one by a thousandth of a point. The refiner's
operation requires the distinction below. The convexity
...
The proposition covers the full-map average $f_S$. A sufficient condition
for the implemented supplied-field average to equal $f_S$ is
$h(Su)=T h(u)$; this condition is not established for the fitted kernel.
...
of the load distribution do not establish this property of a fitted kernel
predictor. Nor does training the refiner with randomly reflected triples
force exact equivariance. We make no non-increase guarantee for its recorded
averaging operation without that additional condition.

There is a quantitative way to state the missing requirement. Write
...
Neither this defect nor such a Lipschitz bound was measured in the campaign,
so this inequality does not certify its observed error changes.

The distinction can matter even with an exactly symmetric target. Let the
...
\eqref{eq:refiner-defect} (here $L_h=1$). It is not a model of the measured
benchmark. The accompanying \texttt{campaign/refiner\_symmetry\_diagnostic.py}
checks the construction and independently recomputes the ten recorded
refiner differences.
