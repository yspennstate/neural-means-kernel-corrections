...
follows from the differentiability assumptions: for fixed $(B,C)$,
differentiable feature and kernel maps make
$\theta\mapsto e_2(\theta;C)$ differentiable wherever the kernel matrix in
$I_C$ remains positive definite. Under the stated local integrable
domination, the derivative passes under the expectation, so
...
$\widehat w_j$ and $w^\ast$ alike, and the preceding inequalities hold with the
additional term $\lambda W^2$. \qed
...
By the argument of Proposition~\ref{prop:affine}, for the class
indexed by $w$ in the $W$-ball and fixed $(j,\gamma)$, the
...
so multiplying by the weights preserves these inequalities:
\[
...
$w_j^2$ leaves every coordinate minimizer unchanged, so the optimizer is the same
...
so evaluation of $Tf$ at $x$ is a bounded functional. The kernel of $T$ is
the intersection of the kernels of these bounded evaluations and is closed.
Its orthogonal complement therefore contains a unique minimum-norm preimage
of every element in the image. Thus the image
...
The quotient by this closed subspace is complete. It is an RKHS, with reproducing kernel
...
Fix $u$ and write $k_u=k(X,u)\in\mathbb R^n$.
At $\lambda=0$, interpret the inverse as $K^+$ if $K$ is singular.
For $z\in\ker K$, the RKHS function $h_z=\sum_i z_i k(u_i,\cdot)$
has squared norm $z^\top Kz=0$. Hence
$z^\top k_u=\langle h_z,k(u,\cdot)\rangle=0$, which places $k_u$ in
$\operatorname{range}K$. In particular, $KK^+k_u=k_u$, so the algebra
below also applies under the pseudoinverse convention.

Put $A=K+n\lambda I$ and $w=A^{-1}k_u$.
For each component $j$, membership $r_j\in\mathcal H_k$ and
...
The argument applies for every $\lambda\ge0$. The inequality between
$\widetilde P_\lambda$ and $P_\lambda$ compares the two factors at the same
ridge value; Theorem~\ref{thm:minimax} describes how that value affects the
worst-case error.
