...
ax.set_title(f"evaluation cases (Pearson {pr:.2f}, Spearman {sr:.2f})")
ax = axes[1]
...
ax.set_title(f"decile calibration, absolute error (Pearson {pa:.2f})")
ax = axes[2]
...
ax.set_title(f"per-decile coverage; overall {100 * cov:.1f}%")
fig.tight_layout()
