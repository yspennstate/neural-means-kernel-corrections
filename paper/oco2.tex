...
Three details of the comparison belong next to these numbers. The combination
...
variability, not that comparison. The comparison concerns the two criteria defined here on the released
reduced representation (the coefficients, and the radiance reconstructed
from the retained forty components) rather than the full-spectrum acceptance
test of \citet{susiluoto2025radiative}, with the networks at a $250$-epoch
...
retrieval itself are outside its scope. Learning the metric rather than
...
than binomial samples. What degrades in the small-margin pairs is the
transfer of two estimates from one block to another, not the proposition.
